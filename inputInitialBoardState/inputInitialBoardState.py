...
def tryToReadNaturalNum(word) -> (int, str):
    if not word.isdigit():
...
def readIntegerSafely(message) -> int:
    ...
    while True:
        print(message, end="")
        word = input()
        number, err = tryToReadNaturalNum(word)
        if err == ReadNaturalNumFuncErrMsgs.ALL_FINE_MSG.value:
            break
...
def readExitGatePos(width, height) -> int:
    ...
    while True:
        ...
        if   sideType == BoardSideTypes.TOP:
            ...
            ind = 2 * width - gateInd + height + 2
        elif sideType == BoardSideTypes.LEFT:
            ind = 2 * (width + height) - gateInd + 3
