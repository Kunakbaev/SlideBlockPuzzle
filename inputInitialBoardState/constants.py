from boardState.constants import MAX_BOARD_WIDTH, MAX_BOARD_HEIGHT, MAX_BOARD_SCALE_FACTOR, \
                                 MAX_NUMBER_OF_BLOCKS, MIN_BLOCK_SIDE_LEN
...
gateInd   - what's index of exit gate. If side type is "top" or "bottom" than index is counted from left to right. Otherwise, it's counted from top to bottom.
    ...
    BOARD_SCALE_FACTOR_INPUT_PROMPT      = "Input board scale factor (size of one tile): "
    ...
    READ_INIT_BOARD_STATE_HELP_MSG       = """Now you need to specify blocks positions on the board.
...
class ReadNaturalNumFuncErrMsgs(StrEnum):
    ...
    ALL_FINE_MSG             = ""
    BOARD_SCL_FACTOR_TOO_BIG = f"Error: board's scale factor is too big, it should not be bigger than {MAX_BOARD_SCALE_FACTOR}"
    BOARD_WIDTH_TOO_BIG      = f"Error: board's width is too big, it should not be bigger than {MAX_BOARD_WIDTH}"
    BOARD_HEIGHT_TOO_BIG     = f"Error: board's height is too big, it should not be bigger than {MAX_BOARD_HEIGHT}"
...
class ReadNewBlockFuncErrMsgs(StrEnum):
    ...
    TOO_MANY_BLOCKS      = f"Error: too many blocks, max number of blocks : {MAX_NUMBER_OF_BLOCKS}"
